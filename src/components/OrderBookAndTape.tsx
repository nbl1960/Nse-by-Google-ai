      ...
      <div className="flex items-center justify-between border-b border-slate-800 px-3 py-2 bg-[#0d121c]">
        ...
        <div className="flex items-center gap-1.5 text-[10px] font-mono text-slate-400">
          ...
          <span className="text-cyan-300 font-bold">
            ₹{spread.toFixed(2)} ({meta.tickSize} tick)
          </span>
        </div>
      </div>
          ...
          <div className="flex flex-col h-full font-mono text-[11px]">
            ...
            <div className="flex-1 flex flex-col justify-end space-y-0.5 overflow-hidden">
              {asks.slice(-6).map((ask, i) => (
                <div key={i} className="relative grid grid-cols-3 px-1 py-0.5 text-rose-300 items-center">
                  ...
                  />
                  <span className="relative z-10 font-medium">₹{ask.price.toFixed(2)}</span>
                  <span className="relative z-10 text-right text-slate-300">{ask.amount.toLocaleString('en-IN')}</span>
            ...
            <div className="my-1.5 py-1 px-2 bg-[#090d14] border-y border-slate-800/80 flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <span className="font-bold text-xs text-slate-100">₹{currentPrice.toFixed(2)}</span>
              </div>
            ...
            <div className="flex-1 space-y-0.5 overflow-hidden">
              {bids.slice(0, 6).map((bid, i) => (
                <div key={i} className="relative grid grid-cols-3 px-1 py-0.5 text-emerald-300 items-center">
                  ...
                  />
                  <span className="relative z-10 font-medium">₹{bid.price.toFixed(2)}</span>
                  <span className="relative z-10 text-right text-slate-300">{bid.amount.toLocaleString('en-IN')}</span>
            ...
            <div className="flex-1 overflow-y-auto space-y-1 pr-1 scrollbar-thin">
              {trades.map((t) => (
                ...
                >
                  ...
                  <span className={`text-right font-bold ${t.side === 'BUY' ? 'text-emerald-400' : 'text-rose-400'}`}>
                    ₹{t.price.toFixed(1)}
                  </span>
